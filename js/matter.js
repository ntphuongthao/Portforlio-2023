const splitWords = () => {
  const textNode = document.getElementById("matter-js");
  const text = textNode.textContent;
  const newDomElements = text.split(" ").map((text) => {
    const highlighted =
      text.startsWith(`technology`) ||
      text.startsWith(`programming`) ||
      text.startsWith(`Software`) ||
      text.startsWith(`Engineering`) ||
      text.startsWith(`Vietnamese`) ||
      text.startsWith(`DePauw`) ||
      text.startsWith(`University`);
